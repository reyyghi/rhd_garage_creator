...
local function updateText()
	local text = {
        "------ Zone Creator ------  \n",
        "X coord [Left/Right]: %s  \n",
        "Y coord [Up/Down]: %s  \n",
        "Z coord [R/F]: %s  \n",
        "Height [Shift + Scroll]: %s  \n",
        "Create new point - [Space]  \n",
        "Edit last point - [Backspace]  \n",
        "Save - [Enter]  \n",
        "Cancel - [Esc]  \n"
	}
...
function CreateZone( cb )
	...
	xCoord = math.round(coords.x) + 0.0
	yCoord = math.round(coords.y) + 0.0
	zCoord = math.round(coords.z) + 0.0
	heading = 0.0
	height = 4.0
	width = 4.0
	length = 4.0
    ...
    while creatorActive do
        ...
        if displayMode == 3 or displayMode == 4 then
            if alignMovementWithCamera then
                ...
                DrawLine(xCoord, yCoord, zCoord, rightX --[[@as number]], rightY or 0, zCoord, 0, 255, 0, 225)
                DrawLine(xCoord, yCoord, zCoord, forwardX --[[@as number]], forwardY or 0, zCoord, 0, 255, 0, 225)
            end
        ...
        if controlsActive then
            ...
            elseif IsDisabledControlJustReleased(0, 22) then -- space
                ...
                coords = newC
                xCoord = math.round(coords.x)
                yCoord = math.round(coords.y)
